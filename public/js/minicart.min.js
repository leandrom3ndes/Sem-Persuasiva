...
!function a(b, c, d) {
    ...
    10: [function(a, b) {
        ...
        var c = a("./util/mixin")
          , d = b.exports = {
            ...
            target: "",
            duration: 30,
            template: '<%var items = cart.items(); var settings = cart.settings();var hasItems = !!items.length;var priceFormat = { format: false, currency: cart.settings("currency_code") };var totalFormat = { format: false, showCode: false };%><form  class="<% if (!hasItems) { %>minicart-empty<% } %>" onsubmit="return false;">    <button type="button" class="minicart-closer">&times;</button>  <ul>    <% for (var i= 0, idx = i + 1, len = items.length; i < len; i++, idx++) { %>        <li class="minicart-item">            <div class="minicart-details-name">                <a class="minicart-name" href="<%= items[i].get("href") %>"><%= items[i].get("item_name") %></a>   <span id="origem-produto"><%= items[i].get("origem") %></span>            <ul class="minicart-attributes">                    <% if (items[i].get("item_number")) { %>                    <li>                        <%= items[i].get("item_number") %>                        <input type="hidden" name="item_number_<%= idx %>" value="<%= items[i].get("item_number") %>" />                    </li>                    <% } %>                    <% if (items[i].discount()) { %>                    <li>                        <%= config.strings.discount %> <%= items[i].discount(priceFormat) %>                        <input type="hidden" name="discount_amount_<%= idx %>" value="<%= items[i].discount() %>" />                    </li>                    <% } %>                    <% for (var options = items[i].options(), j = 0, len2 = options.length; j < len2; j++) { %>                        <li>                            <%= options[j].key %>: <%= options[j].value %>                            <input type="hidden" name="on<%= j %>_<%= idx %>" value="<%= options[j].key %>" />                            <input type="hidden" name="os<%= j %>_<%= idx %>" value="<%= options[j].value %>" />                        </li>                    <% } %>                </ul>            </div>            <div class="minicart-details-quantity">  <div class="change-qt-buttons"><button class="add-qt">+</button><button class="remove-qt">-</button></div>   <div><strong>Qt:</strong><input class="minicart-quantity" data-minicart-idx="<%= i %>" name="quantity_<%= idx %>" type="text" pattern="[0-9]*" value="<%= items[i].get("quantity") %>" autocomplete="off" />            </div>            <div class="minicart-details-remove">                <button type="button" class="minicart-remove" data-minicart-idx="<%= i %>">&times;</button>            </div>            <div class="minicart-details-price">                <span class="minicart-price"><%= items[i].total(priceFormat).toFixed(2) %> €</span>            </div>            <input type="hidden" name="item_name_<%= idx %>" value="<%= items[i].get("item_name") %>" />            <input type="hidden" name="amount_<%= idx %>" value="<%= items[i].amount() %>" />            <input type="hidden" name="shipping_<%= idx %>" value="<%= items[i].get("shipping") %>" />            <input type="hidden" name="shipping2_<%= idx %>" value="<%= items[i].get("shipping2") %>" />        </li>        <% } %>    </ul>    <div class="minicart-footer">        <% if (hasItems) { %>   <div class="discount-button" ><button title="10% desconto - pelo menos metade dos produtos nacionais">Aplicar desconto</button></div>         <div class="minicart-subtotal">                <%= config.strings.subtotal %> <%= cart.total(totalFormat).toFixed(2) %> €           </div>              <% } else { %>            <p class="minicart-empty-text"><%= config.strings.empty %></p>        <% } %>    </div>    <input type="hidden" name="cmd" value="_cart" />    <input type="hidden" name="upload" value="1" />    <% for (var key in settings) { %>        <input type="hidden" name="<%= key %>" value="<%= settings[key] %>" />    <% } %> <% if (hasItems) { %>  <button class="finish-checkout" onclick="handcraftedCheckout()">Checkout</button> <% } %> </form>',
            styles: '.change-qt-buttons button {width: 15px;padding:0px;margin-top:2px; height:min-content; background: #3399cc;border: 1px solid #3399cc;color:white;border-radius:3px;}.change-qt-buttons{display: flex; flex-flow:column;margin-right: 5px;}.discount-button button{border: none;padding: 6px 6px;color: #fff;background: #3399cc;} .discount-button{display:none;} #PPMiniCart .minicart-attributes { display: none; visibility: hidden } .finish-checkout { border: none;padding: 10px 0;color: #fff;width: 100%;background: #3399cc;margin: 1.5em 0 0;} .finish-checkout:hover {background:#fe9126;} @keyframes pop-in {    0% { opacity: 0; transform: scale(0.1); }    60% { opacity: 1; transform: scale(1.2); }    100% { transform: scale(1); }} @-webkit-keyframes pop-in {    0% { opacity: 0; -webkit-transform: scale(0.1); }    60% { opacity: 1; -webkit-transform: scale(1.2); }    100% { -webkit-transform: scale(1); }}@-moz-keyframes pop-in {    0% { opacity: 0; -moz-transform: scale(0.1); }    60% { opacity: 1; -moz-transform: scale(1.2); }    100% { -moz-transform: scale(1); }}.minicart-showing #PPMiniCart {  display: block;    transform: translateZ(0);    -webkit-transform: translateZ(0);    -moz-transform: translateZ(0);    animation: pop-in 0.25s;    -webkit-animation: pop-in 0.25s;    -moz-animation: pop-in 0.25s;}#PPMiniCart {    z-index: 1000; display: none;    position: fixed;    left: 50%;    top: 75px;}   #PPMiniCart form {  position: relative;    width: 400px;    max-height: 400px;    margin-left: -200px;    padding: 10px 10px 40px;    background: #fbfbfb;    border: 1px solid #d7d7d7;    border-radius: 4px;    box-shadow: 2px 2px 10px rgba(0, 0, 0, 0.5);    font: 15px/normal arial, helvetica;    color: #333;}#PPMiniCart form.minicart-empty {    text-align: center; padding-bottom: 10px;    font-size: 16px;    font-weight: bold;}#PPMiniCart ul {    clear: both;    float: left;    width: 380px;    margin: 5px 0 20px;    padding: 10px;    list-style-type: none;    background: #fff;    border: 1px solid #ccc;    border-radius: 4px;    box-shadow: 1px 1px 3px rgba(0, 0, 0, 0.2);}#PPMiniCart .minicart-empty ul {    display: none;}#PPMiniCart .minicart-closer {    float: right;    margin: -12px -10px 0;   background: 0;    border: 0;    font-size: 18px;    cursor: pointer;    font-weight: bold;}#PPMiniCart .minicart-item {    clear: left;    padding: 6px 0;    min-height: 25px;}#PPMiniCart .minicart-item + .minicart-item {    border-top: 1px solid #f2f2f2;}#PPMiniCart .minicart-item a {    color: #333;    text-decoration: none;}#PPMiniCart .minicart-details-name {    float: left;    width: 62%;}#PPMiniCart .minicart-details-quantity {    display:flex;float: left;    width: 15%;}#PPMiniCart .minicart-details-remove {    position:absolute;right:2%;    width: 7%;}#PPMiniCart .minicart-details-price {    width: fit-content; position: absolute;right: 15%;    text-align: right;}#PPMiniCart .minicart-attributes {    margin: 0;    padding: 0;    background: transparent;    border: 0;    border-radius: 0;    box-shadow: none;    color: #999;    font-size: 12px;    line-height: 22px;}#PPMiniCart .minicart-attributes li {    display: inline;}#PPMiniCart .minicart-attributes li:after {    content: ",";}#PPMiniCart .minicart-attributes li:last-child:after {    content: "";}#PPMiniCart .minicart-quantity {    width: 30px;    height: 18px;    padding: 2px 4px;    border: 1px solid #ccc;    border-radius: 4px;    box-shadow: inset 0 1px 1px rgba(0, 0, 0, 0.075);    font-size: 13px;    text-align: left;    transition: border linear 0.2s, box-shadow linear 0.2s;    -webkit-transition: border linear 0.2s, box-shadow linear 0.2s;    -moz-transition: border linear 0.2s, box-shadow linear 0.2s;}#PPMiniCart .minicart-quantity:hover {    border-color: #0078C1;}#PPMiniCart .minicart-quantity:focus {    border-color: #0078C1;    outline: 0;    box-shadow: inset 0 1px 1px rgba(0, 0, 0, 0.075), 0 0 3px rgba(0, 120, 193, 0.4);}#PPMiniCart .minicart-remove {    width: 18px;    height: 19px;    margin: 2px 0 0;    padding: 0;    background: #b7b7b7;    border: 1px solid #a3a3a3;    border-radius: 3px;    color: #fff;    font-size: 13px;    opacity: 0.70;    cursor: pointer;}#PPMiniCart .minicart-remove:hover {    opacity: 1;}#PPMiniCart .minicart-footer {    clear: left;width: 100%;}#PPMiniCart .minicart-subtotal {    position: absolute;    bottom: 17px;    padding-left: 6px;    right:1%;    font-size: 16px;    font-weight: bold;}#PPMiniCart .minicart-submit {    position: absolute;    bottom: 10px;    right: 10px;    min-width: 153px;    height: 33px;    margin-right: 6px;    padding: 0 9px;    border: 1px solid #ffc727;    border-radius: 5px;    color: #000;    text-shadow: 1px 1px 1px #fff6e9;    cursor: pointer;    background: #ffaa00;    background: url(data:image/svg+xml;base64,);    background: -moz-linear-gradient(top, #fff6e9 0%, #ffaa00 100%);    background: -webkit-gradient(linear, left top, left bottom, color-stop(0%,#fff6e9), color-stop(100%,#ffaa00));    background: -webkit-linear-gradient(top, #fff6e9 0%,#ffaa00 100%);    background: -o-linear-gradient(top, #fff6e9 0%,#ffaa00 100%);    background: -ms-linear-gradient(top, #fff6e9 0%,#ffaa00 100%);    background: linear-gradient(to bottom, #fff6e9 0%,#ffaa00 100%);}#PPMiniCart .minicart-submit img {    vertical-align: middle;    padding: 4px 0 0 2px;}',
            strings: {
                button: '',
                subtotal: "Total:",
                discount: "Discount:",
                empty: "Não tem nada no carrinho!"
            }
        };
    ...
    22: [function(a, b) {
        ...
        function c(a) {
            ...
            // e.add(this.el.querySelectorAll(".add-qt"), "keyup", i.keyup, this),
        ...
        c.prototype.redraw = function() {
            ...
            updateTotalSpan(getCartAmount(this.model.cart._items)),
            getCartQuantity(this.model.cart._items),
            itemChange(this.model.cart._items)
        ...
        c.prototype.show = function() {
            event ? triggerIsCart(event.target) ? (
                this.isShowing || (h.add(document.body, j.SHOWING_CLASS),
                this.isShowing = !0
            )) :  null : null;
            disableInput(this.model.cart._items);
            $(".discount-button").hide();
            discountBonus(this.model.cart._items);
        }
        ,
        ...
        b.exports = c = {
            click: function(a) {
                ...
                if (this.isShowing)
                    if (c === d.CLOSER_CLASS)
                        ...
                        b[b.setSelectionRange ? "setSelectionRange" : "select"](0, 50);
                    
                    // + and - buttons
                    else if (c === "add-qt") {
                        var previousValue = Number($(b).parent().parent().find(".minicart-quantity").val());
                        var position = $(b).parent().parent().find(".minicart-quantity").attr("data-minicart-idx");
                        var cartItems = this.model.cart._items;
                        var itemToChange = cartItems[position];
                        itemToChange.set("quantity", previousValue + 1);
                    }
            ...
            keyup: function(a) {
                ...
                e.className === d.QUANTITY_CLASS && (b = setTimeout(function() {
                    ...
                    f && 
                    (g > 0  ? f.set("quantity", g) : 0 === g && b.remove(a));
                }, d.KEYUP_TIMEOUT))
...
function handcraftedCheckout() {
    var products = [];

    $(".minicart-item").each( function() {

        var product = $(this).find(".minicart-name").html();
        var quantity = $(this).find(".minicart-quantity").attr("value");
        var origem = $(this).find("#origem-produto").html();
    ...
    localStorage.setItem("produtos", encodeURIComponent(JSON.stringify(products)));

    window.location.href = './dadosCheckout.html';
}

function hasDiscount(cartItems) {
    var totalGoodies = 0;
    var nationalGoodies = 0;
    cartItems.forEach( function( item ) {
        totalGoodies += item._data.quantity;
        item._data.origem == "Nacional" ? nationalGoodies += item._data.quantity : null;
    })
    return (totalGoodies / 2 <= nationalGoodies);
}

function discountBonus(cartItems) {
    if (hasDiscount(cartItems)) {
        $(".discount-button").show();
        $(".discount-button button").click( function() {
            $(".minicart-subtotal").html("Total: " + (getCartAmount(cartItems) * 0.9).toFixed(2) + " €");
            disableButton(this);
        });
    }
}
...
function disableButton(button) {
    ...
    $(button).html("Desconto aplicado");
    $(button).css("background-color", "#fe9126");
}

function enableButton(button) {
    $(button).prop("disabled", false);
    $(button).html("Aplicar desconto");
}

function triggerIsCart(element) {
    var result;
    $(element).find(".w3view-cart")[0] ? result = true : result = false;
    return result;
}

function hideButtonsCheckout() {
    $(".minicart-footer").hide();
    $(".finish-checkout").hide();
    $(".change-qt-buttons").each( function() {
        $(this).hide();
    });
    $(".minicart-details-remove").each( function() {
        $(this).hide();
    });
}

function disableInput(cartItems) {
    
    $(".minicart-name").each( function() {
        $(this).css("pointer-events", "none");
    });

    $(".minicart-details-quantity .minicart-quantity").each( function() {
    ...
    var amount = getCartAmount(cartItems);
    amount >= cashAmount ? 
        ( $(".add-qt").prop("disabled", true) ,
          $(".add-qt").css("background", "none"),
          $(".add-qt").css("border", "none"),
          $(".add-qt").css("background-color", "grey")
        )
        : null;

    window.location.href.includes("dadosCheckout") ? hideButtonsCheckout() : null;
}
...
function itemChange(cartItems) {
    var result = getCartAmount(cartItems);
    updateTotalSpan(result);
    getCartQuantity(cartItems);
    result > cashAmount && !window.location.href.includes("dadosCheckout") ? disableButtons() : enableButtons();
}

function getCartQuantity(cartItems) {
    var result = 0;
    cartItems.forEach( function(item) {
        result += item._data.quantity;
    })
    $("#number-goodies").html(result);
}

function updateTotalSpan(total) {
    var value = cashAmount - total > 0 ? cashAmount - total : 0;
    $(".w3ls_logo_products_left span").html(value.toFixed(2) + "€");
}

function disableButtons() {
    $("form input[type=submit]").each( function() {
        $(this).attr("value", "Checkout");  
        $(this).prop('disabled', true);
        $(this).css('cursor', 'not-allowed');
    });

    // blinking cart
    $(".fa-cart-arrow-down").addClass("blinking-cart");
    $(".notice-checkout").css("display", "block");
    $(".notice-checkout").addClass("blinking-cart");
}
...
function enableButtons() {
    $("form input[type=submit]").each( function() {
        ...
        $(this).attr("value", "Adicionar");  
    });
    $(".fa-cart-arrow-down").removeClass("blinking-cart");
    $(".notice-checkout").css("display", "none");
    $(".notice-checkout").removeClass("blinking-cart");
}


function completeCheckout() {
    // check if required fields were filled
    var regiao = $("#input-regiao").val();
    var sexo = $("#input-sexo").val();
    var idade = $("#input-idade").val();
    var canPost = true;

    !idade || !(15<= idade && idade <= 99) || !regiao || !sexo ? canPost = false : null;

    // get data from local storage
    var produtos = JSON.parse(decodeURIComponent(localStorage.getItem("produtos")));
    ...
    if ( canPost ) {
        axios.post("https://persuasive-local-commerce.firebaseio.com/compras.json", data)
        .then( function(response) {
            alert("Obrigado pela participacao! Clica OK para finalizar.");
            localStorage.removeItem("produtos");
            localStorage.removeItem("PPMiniCart");
            location.replace("https://www.google.pt/");
        })
...
function flagOnlyOnNational() {
    $(".agile_top_brand_left_grid").each( function() {
        $(this).find("form input[name='origem']").val() == 'Importado' 
        ?   $(this).find(".agile_top_brand_left_grid_pos").hide() : null;
    });
}
